...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	...
	else if(FontSize == OLED_16X32)   // 新增
    {
        /* 注意：只支持 0-9 和 ':' ，其他字符显示默认 */
        if (Char >= '0' && Char <= '9')
            OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[Char - '0']);
        else if (Char == ':')
            OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[10]);
		else if (Char == 'O')
			OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[11]);
		else if (Char == 'F')
			OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[12]);
		else if (Char == 'N')
			OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[13]);
		else if (Char == ' ')
			OLED_ShowImage(X, Y, 16, 32, OLED_F16x32[14]);
        else
            OLED_ShowChar(X, Y, '?', OLED_8X16); // 退回默认
    }
}
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		else					//否则，即多字节字符
		{
			...
			else if (FontSize == OLED_16X32)
			{
				OLED_ShowChar(X + XOffset, Y, '?', OLED_8X16);  // 用8x16显示问号
				XOffset += OLED_8X16;
			}
		}
	}
...
/**
 * 函    数：OLED显示电池图标（三格满格图，根据电量擦除多余格）
 * 参    数：percent 电量百分比 0~100
 * 返 回 值：无
 * 说    明：满格电池图标需在 OLED_Data.c 中定义为 BatteryFull
 */
void OLED_ShowBattery(uint8_t percent)
{
    // 1. 绘制满格电池图标
    OLED_ShowImage(110, 0, 16, 16, BatteryFull);
    ...
    // 3. 擦除多余的格子
    //    假设图标从左到右依次是：电池壳 + 格1 + 格2 + 格3
    //    以下坐标根据你的满格图标布局微调
    if (bars < 3) OLED_ClearArea(119, 5, 2, 6);  // 擦除最右一格
    if (bars < 2) OLED_ClearArea(116, 5, 5, 6);  // 擦除右侧两格
    if (bars < 1) OLED_ClearArea(113, 5, 8, 6);  // 全部三格擦除，仅留电池壳
}
